...
| | ESP8266 Arduino | ESP32 Arduino | ESP32 MicroPython |
|---|---|---|---|
| Wi-Fi header | `<ESP8266WiFi.h>` | `<WiFi.h>` | `network` module |
| ESP-NOW header | `<espnow.h>` | `<esp_now.h>` + `<esp_wifi.h>` | `espnow` module |
| Channel setting | `wifi_set_channel(4)` (SDK) | `esp_wifi_set_channel(4, WIFI_SECOND_CHAN_NONE)` | `wlan.config(channel=4)` |
| Role setup | `esp_now_set_self_role(ESP_NOW_ROLE_SLAVE)` | Not needed | Not needed |
| Callback style | `esp_now_register_recv_cb()` | `esp_now_register_recv_cb()` | `e.irq()` |
| Broadcast peer registration | Not needed | Not needed | **Required** — `e.add_peer(b'\xff\xff\xff\xff\xff\xff')` |
